...
void EqHandle::paint( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget )
{
	...
	if ( isMouseHover() )
	{
		...
		if ( EqHandle::y() < 50 )
		{
			rectY = rectY + 50 - EqHandle::y();
		}
		if ( EqHandle::x() < 50 )
		{
			rectX = rectX + 50 - EqHandle::x();
		}
		if ( EqHandle::x() > m_width - 50 )
		{
			rectX = rectX - ( 50 - ( m_width - EqHandle::x() ) );
		}
		QPixmap hover = PLUGIN_NAME::getIconPixmap( "handlehover" );
		painter->drawPixmap( - ( hover.width() / 2) - 1, - ( hover.height() / 2 ), hover );
		QRectF textRect = QRectF ( rectX, rectY, 150, 80 );
		...
		painterFont.setPointSizeF( painterFont.pointSizeF() * 1.0 );
...
void EqCurve::paint( QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget )
{
	...
	for ( int i = 0; i < m_handle->count(); i++ )
	{
		...
		{
			curveColor = QColor( qRgba( 200, 200, 200, 200 ));
			QPen pen ( curveColor);
			pen.setWidth( 2 );
			painter->setPen( pen );
